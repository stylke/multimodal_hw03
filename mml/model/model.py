...
class Net(nn.Module):
    ...
    def __init__(
        ...
        # print(f"self.mp embed_size: {self.ie.model.config.hidden_size}")
    ...
    def train_forward(self, img_emb, trg_cap, att_mask):
        ...
        # print(f"img_emb dtype: {img_emb.dtype}")
        img_mapped = self.mp(img_emb, train_mode=True)
        # print(f"img_mapped shape: {img_mapped.shape}")
        ...
        # res = torch.softmax(res, dim=2)  不要这行！！！
