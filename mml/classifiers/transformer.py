...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        caption_embeddings = self.embedding(captions)  # Shape: (N, T, embed_dim)
        caption_embeddings = self.positional_encoding(caption_embeddings)  # Shape: (N, T, embed_dim)

        image_features = self.visual_projection(features).unsqueeze(1)  # Shape: (N, 1, embed_dim)
        ...
        scores = self.output(decoder_output)  # Shape: (N, T, vocab_size)
