...
from utils import ConfigS, ConfigL
...
def evaluate_dataset(model, dataset, img_path, save_path, temperature=1.0):
    ...
    for img_data in loop:
        ...
        print(f"img_name: {img_name}, caption: {caption}")
...
if __name__ == "__main__":
    ...
    #     device=device,
    # )
    model = torch.load("./model_epoch_8.pt").to(device)

    # TODO: 需要你自己实现一个ImageCaptionDataset在`data/dataset.py`中
    dataset = ImageCaptionDataset("./data/image_datas_base32.pkl")
