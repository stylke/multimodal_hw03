...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        queries = self.query(query).reshape(N, S, H, E // H)
        keys = self.key(key).reshape(N, T, H, E // H)
        values = self.value(value).reshape(N, T, H, E // H)

        # print(f"query shape: {query.shape}")

        energy = torch.matmul(queries.permute(0, 2, 1, 3), keys.permute(0, 2, 3, 1)) / (self.head_dim ** 0.5)

        if attn_mask is not None:
            # print(f"attn_mask shape: {attn_mask.shape}, energy shape: {energy.shape}")
            energy = energy.masked_fill(attn_mask == 0, float("-inf"))

        attention = self.attn_drop(torch.softmax(energy, dim=-1))  # shape (N, H, S, T)

        output = torch.matmul(attention, values.permute(0, 2, 1, 3)).permute(0, 2, 1, 3).reshape(N, S, E)
