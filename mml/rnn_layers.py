...
def rnn_step_backward(dnext_h, cache):
    ...
    dtanh = dnext_h * (1 - np.tanh(prev_h @ Wh + x @ Wx + b)**2)
    dx = dtanh @ Wx.T
    dprev_h = dtanh @ Wh.T
    dWx = x.T @ dtanh
    dWh = prev_h.T @ dtanh
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        ...
        dx_step, dprev_h, dWx_step, dWh_step, db_step = rnn_step_backward(dnext_h, step_cache)
        dx[:, t, :] = dx_step
        dWx += dWx_step
        dWh += dWh_step
        db += db_step
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        step_cache = cache[t]
        dnext_h = dh[:, t, :] + dprev_h
        dx_step, dprev_h, dprev_c, dWx_step, dWh_step, db_step = lstm_step_backward(dnext_h, dprev_c, step_cache)
        dx[:, t, :] = dx_step
        dWx += dWx_step
        dWh += dWh_step
        db += db_step
